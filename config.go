...
func parse(cmdline []string) (parameters, error) {
	...
	var overwriteBucket = flags.Int("overwritebucket", 0, "Turns an operation on the same s3 bucket. (1: 1 bucket per worker, 2: 1 bucket per object, default: everything in one bucket).")
	...
	flags.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "Bucket naming is unique unless the 'overwrite_default' option is used. The naming is as follows:\n")
		fmt.Fprintf(os.Stderr, "    default: 1 bucket for all requests (\"<bucket>\")\n")
		fmt.Fprintf(os.Stderr, "    1:       1 bucket per worker (\"<bucket>-N\" where N is 0..concurrency-1)\n")
		fmt.Fprintf(os.Stderr, "    2:       1 bucket per request (\"<bucket>-N-M\" where N is 0..concurrency-1 and M is the request within that connection)\n")
	...
	if !opTypeExists {
		var regex = fmt.Sprintf("^((%s)[1-9]\\d?)+$", strings.Join(optypes, "|"))
		matched, _ := regexp.MatchString(regex, *optype)
		if !matched {
			return parameters{}, fmt.Errorf("operation type must be one of: %s or a mix like 'put20get75delete5'", operationListString)
		}
		re := regexp.MustCompile(`[1-9]\d?`)
		var ratios = re.FindAllString(*optype, -1)
		var i int
		var total int = 0
		for i=0; i<len(ratios); i++ {
			ratio, _ := strconv.Atoi(ratios[i])
			total += ratio
		}
		if total != 100 {
			return parameters{}, fmt.Errorf("mix operations total ratios must equal 100")
		}
	}
	...
	args := parameters{
		...
		overwrite:          *overwrite,
		overwriteBucket:   *overwriteBucket,
		retries:            *retries,
